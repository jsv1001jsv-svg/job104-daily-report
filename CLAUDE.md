...
> 狀態：**本機端到端已跑通 —— 日報實際推送到手機成功**（2026-08-06）。
> 抓取 → 去重 → LLM 摘要 → LINE 推播全鏈路以真實服務驗證，覆蓋率 99%。
> 剩下：webhook 實測（需公開網址）與雲端部署（Firebase + Modal 帳號）。
> 最後更新：2026-08-06
...
**現階段只服務作者本人**，webhook 那條路（別人加好友自動註冊）暫不啟用。
程式碼保留且單元測試覆蓋率 100%，只是不對真實 LINE 驗證、不填 Webhook URL。

| | 完整版（原規劃） | **現階段** |
| :--- | :--- | :--- |
| 使用者 | 任何人加好友即可用 | 只有作者 |
| 條件設定 | LINE 傳訊息 | `scripts/seed_user.py` 手動寫入 |
| Webhook | 需驗證 | 保留程式碼，不啟用 |
| Firebase | 需要 | **仍然需要**（去重靠它） |

**理由**：先把「每天早上真的收到日報」這件事跑順，再擴大到多使用者。
webhook 的第一次真實驗證會發生在雲端（除錯迴圈較慢），沒必要和部署驗證
綁在同一輪一起賭。

⚠️ Firebase 兩種版本都躲不掉 —— 「只推今天新出現的職缺」是第 2 節的核心規則，
需要地方記住「已看過哪些」。
...
- 🎉 **端到端跑通，日報已實際推到手機**。三條路徑全驗證：首次 20 筆、
  去重後 2 筆、零新職缺推提示訊息（且不呼叫 LLM）
- **批次摘要**（原待辦，實測 429 後變成阻塞問題）：免費層以「請求數」計限，
  原本 20 筆職缺併發 20 個請求必被擋。改為一次請求 10 筆、批次間依序送出，
  **20 個請求降到 2 個**
- **預設模型改別名**：釘死的 `gemini-2.5-flash` 對新帳號回 404「不再開放給
  新使用者」。改用 `gemini-flash-lite-latest` —— 釘版本會無預警失效
- **待辦**：`require_production_secrets()` 全專案沒有任何地方呼叫，
  「缺金鑰就啟動失敗」的保護目前是空的；webhook 尚未實測（需公開網址）
